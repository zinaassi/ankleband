#!/usr/bin/env python3
"""
Generate multi-region comparison plots for EMA alpha=0.3 vs raw signal.
This is the best performing EMA configuration.

Uses the exact same data samples as the Kalman best comparison for direct comparison.
"""
...
# Configuration
SAMPLING_RATE = 200
OUTPUT_DIR = Path('outputs_organized/04_final_visualizations/ema_best_comparison')
DATA_DIR = Path('data/dataset')
IMU_CHANNELS = ['acc_x', 'acc_y', 'acc_z', 'gyro_x', 'gyro_y', 'gyro_z']
CHANNEL_LABELS = ['Acc X', 'Acc Y', 'Acc Z', 'Gyro X', 'Gyro Y', 'Gyro Z']

# Signal configs - EXACT SAME as Kalman best comparison
SIGNAL_CONFIGS = {
...
# EMA configuration - Best performing
EMA_CONFIG = {'alpha': 0.3, 'label': 'EMA (alpha=0.3)', 'color': '#e74c3c'}
...
def plot_multiregion_comparison(signal_label, channel='acc_z'):
    ...
    Matches the Kalman best format exactly.
    """
    ...
    # Apply EMA filter
    ema_filter = EMAFilter(alpha=EMA_CONFIG['alpha'])
    filtered_df = ema_filter.apply(original_df)
    ...
    ax_full.plot(time_axis, filtered_df[channel].values, color=EMA_CONFIG['color'],
                linewidth=1.8, alpha=0.9, label=EMA_CONFIG['label'], zorder=2)
    ...
    ax_quiet.plot(quiet_time, filtered_df[channel].values[quiet_start:quiet_end],
                 color=EMA_CONFIG['color'], linewidth=3, label=EMA_CONFIG['label'],
                 alpha=0.9, zorder=2)
    ...
    # Overall title
    title = f"EMA alpha=0.3 - Multi-Region Comparison\n"
    ...
    # Save
    filename = f"ema_best_multiregion_{channel}_{signal_label}.png"
...
def main():
    ...
    print("\n" + "="*80)
    print("EMA alpha=0.3 - MULTI-REGION COMPARISON")
    print("Best Performing EMA Configuration")
    print("="*80)
    ...
    print(f"  Alpha (Smoothing Factor): {EMA_CONFIG['alpha']}")
    print(f"  Approximate Cutoff: ~30 Hz at 200 Hz sampling rate")
    print(f"\nThis configuration provides optimal noise reduction while preserving gesture features")

    print(f"\nUsing EXACT SAME data samples as Kalman best comparison:")
    for signal_label, config in SIGNAL_CONFIGS.items():
        print(f"  - {signal_label}: {config['file']}, Gesture {config['gesture']}")
    ...
    print("\nCompare with Kalman best results in:")
    print("  outputs_organized/04_final_visualizations/kalman_best_comparison/")
